...
extension UIColor {
    var appColor : UIColor {
        return UIColor(hexString: "#FEA86D")
    }
}
...
func logOutAndBackToMainScreen() -> Void {
    
    UserDefault.deleteModel(key: userDefaultIdentifier.Accesstoken)
    
    
    let  navigationController = NavigationController(rootViewController: LoginViewController
        .instantiate(fromStoryBoards: .Authentication))
    //Create and set up the animation
    UIApplication.setRootView(navigationController, options: UIApplication.logoutAnimation, animated: true, duration: 1.5, completion: nil)
}

extension UIApplication {
    
    static var loginAnimation: UIView.AnimationOptions = .transitionFlipFromRight
    static var logoutAnimation: UIView.AnimationOptions = .curveEaseIn
